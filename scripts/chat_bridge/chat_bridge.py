...
Generated by the webapp-chat-bridge skill. Per-instance env prefix: THERMOMIX.

Env (all optional, sensible defaults baked in at install time):
  THERMOMIX_CHAT_BASE    — webapp base URL (e.g. http://192.168.3.223 or http://localhost:3000)
  CHAT_BRIDGE_TOKEN           — bearer for /api/chat/inbox|reply (global; share with webapp env)
  THERMOMIX_REPO_PATH    — peer's repo path. Used to match the right claude-peers peer.
  THERMOMIX_PEERS_BROKER — Default: http://127.0.0.1:7899
  THERMOMIX_CHAT_POLL_S  — Default: 2
  THERMOMIX_PEER_POLL_S  — Default: 5
  THERMOMIX_QUEUE_DIR    — Default: ~/.thermomix-pending-chat
  THERMOMIX_QUEUE_MAX    — Default: 500 (oldest dropped when exceeded)
  THERMOMIX_CHAT_LOG     — Default: <repo>/scripts/chat_bridge/chat-bridge.log
"""
...
P = "THERMOMIX"  # interpolated by install.sh
...
QUEUE_DIR = Path(os.environ.get(
    f"{P}_QUEUE_DIR",
    str(Path.home() / ".thermomix-pending-chat"),
))
...
def _webapp(method: str, path: str, body=None, timeout: float = 15.0) -> dict:
    ...
    headers = {"User-Agent": "thermomix-chat-bridge/1"}
    if data is not None:
...
def render_for_peer(msg: dict) -> str:
    return (
        ...
        f"#   ./scripts/chat_bridge/thermomix-chat reply \"your answer\"\n"
